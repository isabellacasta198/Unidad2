...
public class MainBinarySearch {
	...
	public static void main(String[] args) {
        ...
        // Búsqueda iterativa
        int resultadoIterativo = busquedaBinariaIterativa(testArray, numeroBuscado);
        if (resultadoIterativo != -1) {
            System.out.println("Encontrado (Iterativo) en la posición: " + resultadoIterativo);
        } else {
            System.out.println("No encontrado (Iterativo).");
        }

        // Búsqueda recursiva
        int resultadoRecursivo = busquedaBinariaRecursiva(testArray, numeroBuscado, 0, testArray.length - 1);
        if (resultadoRecursivo != -1) {
            System.out.println("Encontrado (Recursivo) en la posición: " + resultadoRecursivo);
        } else {
            System.out.println("No encontrado (Recursivo).");
        }
